...
const app = {
    init() {
        try {
            // Immediately initialize theme and direction from localStorage state before anything else
            const savedTheme = localStorage.getItem('nova_theme') || 'light';
            const savedDir = localStorage.getItem('nova_dir') || 'ltr';
            Store.state.theme = savedTheme;
            Store.state.dir = savedDir;
            ...
            this.checkAuth();
            console.log('✅ App initialized with persistent state support');
        } catch (err) {
    ...
    applyTheme() {
        try {
            ...
            // Immediately synchronize icons across all global elements
    ...
    applyDir() {
        try {
            ...
            // Immediately synchronize icon/state for text direction alignment toggle
            const rtlToggleBtn = document.getElementById('rtlToggle');
            if (rtlToggleBtn) {
                rtlToggleBtn.innerHTML = `<span class="material-symbols-outlined">${isRtl ? 'format_align_right' : 'format_align_left'}</span>`;
            }
        } catch (err) {
    ...
    syncGlobalUI() {
        try {
            ...
            // Ensure theme and dir application syncs UI elements correctly
            this.applyTheme();
            this.applyDir();
        ...
        try {
            ...
            if (pages.includes(name)) {
                ...
                if (res.ok) {
                    ...
                    // Re-sync UI state elements after view loads dynamically
